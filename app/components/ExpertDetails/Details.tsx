...
export default function Details() {
  ...
  const handleAddReview = async () => {
    // Validate form
    const newErrors: ErrorState = {};
    if (!newReview.name.trim()) newErrors.name = t("details.modal.name_label") + " " + t("details.modal.required");
    if (!newReview.role.trim()) newErrors.role = t("details.modal.role_label") + " " + t("details.modal.required");
    if (!newReview.comment.trim()) newErrors.comment = t("details.modal.review_label") + " " + t("details.modal.required");
    if (newReview.rating === 0) newErrors.rating = t("details.modal.rating_required");
