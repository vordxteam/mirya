...
export default function Plans() {
  ...
  const currentPlans = planData[activeTab as keyof typeof planData];
  ...
  return (
    <div className='bg-[url("/images/bg-png.png")] bg-no-repeat  bg-top flex items-center justify-center py-[60px] relative'>
      ...
      <div>
        ...
        <h1 className="heading-1 font-medium text-center tracking-[-1.44px] max-w-[430px]  pb-3 sm:pb-6">
          Get your package
        </h1>

        {/* Subheading */}
        <p className="text-[#CAC9D1] text-center sm:text-start heading-6 font-normal  pb-3 sm:pb-6">
          Pick your Plan and Start Unleash your Creativity from now
        </p>
        </div>
        ...
        <motion.div
          ...
          className="grid lg:grid-cols-3 gap-10 pt-[152px] px-10  md::pt-[152px]"
        >
          {currentPlans.map((item, index) => (
            <motion.div
              ...
              className={`bg-[#090b26] max-w-[300px] md:max-w-[396px]  px-[32px] pt-[72px] pb-[48px] rounded- plans relative cursor-pointer transition-all ${
                activePlanId === item.id ? "ring-2 ring-[#4F60FA] -mt-20 shadow-lg shadow-[#4F60FA]/30" : ""
              }`}
            >
                {
            ...
            <div className="rounded-[68.75px] opacity-[0.6] bg-[#4F60FA] blur-[50px] sm:w-[181px] h-[94px]"></div>
              ...
              <div className="mt-3 bg-linear-to-r from-[#1A1A3B] via-[#A68BEE] to-[#1A1A3B] sm:w-[332px] h-px"></div>
              ...
              <GradientButton bgColor={item.btnColor} label={item.btnText} textColor="white" href="#"  />
              </div>
            </motion.div>
          ))}
        </motion.div>
