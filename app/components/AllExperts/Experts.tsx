...
const filters = {
  ...
  services: ["Process Automation", "Digital Transformation", "AI Consulting", "Cloud Services"],
...
export default function Experts() {
  ...
  const transformExpertData = (data: any[]): TransformedExpert[] => {
    ...
    return data.map((item) => {
      ...
      const companyName = recordMap["company_name"] || `${item.first_name || ''} ${item.last_name || ''}`.trim() || 'Unknown';
      const description = recordMap["company_description"] || "No description available.";
      ...
      return {
        ...
        acceptingClients: item.status === 'active' || item.status === 'pending' || true,
      };
    });
  };
  ...
  const fetchExperts = async () => {
    try {
      ...
      if (response && (response.success || response.status)) {
        ...
        if (expertData && typeof expertData === 'object' && 'expert' in expertData) {
  ...
  const StarRating = ({ rating, count }: any) => (
    <div className="inline-flex items-center gap-2 bg-[#FFFFFF1F] px-2 py-1 rounded-full">
      ...
      <span className="heading-7 font-normal text-[#FFFFFF]">{rating} ({count})</span>
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-15">
          ...
          <div className="flex-1">
            ...
            {loading && experts.length === 0 && (
              <div className="flex items-center justify-center py-20">
                <div className="text-center">
                  ...
                  <p className="text-gray-400 heading-5">Loading experts...</p>
                  <p className="text-gray-500 text-sm mt-2">If this takes too long, check your API connection</p>
                </div>
              </div>
            )}
            ...
            {/* Error State */}
            {error && !loading && (
              <div className=" rounded-lg p-6 mb-6 flex justify-center items-center h-full">
              No data found
              </div>
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 z-0">
                  {experts.map((expert) => (
                    <Link 
                      ...
                      style={{
                        background: 'linear-gradient(97deg, #22223C 14.82%, #22223C 25.27%, #686DDD 39.55%, #22223C 49.99%, #22223C 84.47%)'
                      }}
                    >
                          ...
                          <div className="inline-flex items-center gap-2 bg-[#E1F9E11F] rounded-full p-2 mb-4 border border-[#E1F9E166]">
                            <div className="w-4 h-4 bg-[#08CD05] rounded-full flex items-center justify-center">
                              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <path d="M4.875 7.91432L6.63375 9.67307C6.69561 9.73576 6.76931 9.78553 6.85056 9.81951C6.93182 9.85348 7.01901 9.87098 7.10708 9.87098C7.19515 9.87098 7.28235 9.85348 7.3636 9.81951C7.44486 9.78553 7.51856 9.73576 7.58042 9.67307L11.125 6.12891" stroke="white" strokeLinecap="round" strokeLinejoin="round"/>
                              </svg>
                            </div>
                            <span className="heading-7 font-normal text-[#E1F9E1]">Accepting New Clients</span>
                          </div>
                        )}
