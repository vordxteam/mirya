...
export const OnboardingLayout = ({
  ...
  return (
    <div className="bg-[#020817] min-h-screen flex flex-col bg-[url('/images/main-gradient.png')] bg-no-repeat bg-center bg-cover ">
      {/* Main content area */}
      <div className="flex-1 flex items-center justify-center py-8">
        <div className="relative z-10 w-full max-w-[841px] px-6">
                ...
                <div className="flex gap-2 items-center">
                  {[...Array(3)].map((_, i) => {
                    ...
                    const segmentProgress = Math.max(
                      ...
                      Math.min(
                        1,
                        (currentStep - segmentStart) / (segmentEnd - segmentStart),
                      ),
                    );
                    const isCompleted = currentStep > segmentEnd;
                    const isActive = currentStep > segmentStart && currentStep <= segmentEnd;
                      ...
                      <div key={i} className="flex-1 relative h-1">
                        ...
                        {(isCompleted || isActive) && (
                          <div
                            className="absolute inset-0 bg-blue-500 rounded-full transition-all duration-300"
                            style={{
                              width: isCompleted ? "100%" : `${segmentProgress * 100}%`,
                            }}
                          />
                        )}
                      </div>
          ...
          <div className="flex justify-between items-center mt-10">
            <div
              className={`inline-flex rounded-[40px] p-[1px] ${currentStep === 1 ? "invisible" : "visible"}`}
              style={{
                background: "linear-gradient(90deg, #343754 0%, #AAB1EC 50%, #343754 100%)",
              }}
            >
              <button
                onClick={onBack}
            ...
            <div className="flex items-center gap-4">
              <GradientButton
                onClick={onContinue}
                label={currentStep === totalSteps ? t("onboarding.buttons.submit") : t("onboarding.buttons.continue")}
                bgColor="#0274FE"
                textColor="#FFFFFF"
              />
            </div>
      ...
      {/* Footer */}
      <div className="mt-auto max-w-[1440px] mx-auto w-full px-3 sm:px-8 pb-4">
        <div className="flex flex-col sm:flex-row items-center justify-between">
          <div className="heading-6 font-normal text-[#FFFFFFCC] text-xs mb-2 sm:mb-0">
          ...
          <div className="flex gap-3 text-[#FFFFFFCC] text-xs">
            ...
            <Link href="#" className="border-r pr-2 border-[#FFFFFF33]">
              {t("onboarding.footer.security")}
            </Link>
            <Link href="#" className="">
              {t("onboarding.footer.imprint")}
            </Link>
          </div>
        </div>
      </div>
